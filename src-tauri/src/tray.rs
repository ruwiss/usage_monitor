...
fn icon_png(state: &Arc<AppState>) -> Vec<u8> {
    ...
    let top = data.get(&top_field).and_then(|v| v.get("utilization")).and_then(Value::as_f64).unwrap_or(0.0);
    let bottom = data.get(&bottom_field).and_then(|v| v.get("utilization")).and_then(Value::as_f64).unwrap_or(0.0);
